#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""近3日均量/此前20日均量 * (-x)；x = sinh(k·u)，u 为近 100 日高低位。"""
...
class VolRelMa5Factor(BaseFactor):
    ...
    def calculate(
        ...
        ma3 = g_vol.transform(lambda s: s.rolling(3).mean())
        ma20 = g_vol.transform(lambda s: s.shift(3).rolling(20).mean())
